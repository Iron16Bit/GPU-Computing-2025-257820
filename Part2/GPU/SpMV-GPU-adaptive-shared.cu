...
#define SHARED_MEM_ELEMENTS 1024  // Shared memory buffer size
...
void get_enhanced_launch_config(int n, int nnz, const int *row_ptr,
    ...
    printf("  Short rows: %d (%.1f%%) - Direct processing (no shared memory)\n",
           *num_short, 100.0 * (*num_short) / n);
    printf("  Long rows: %d (%.1f%%) - Warp-cooperative with shared memory\n",
           *num_long, 100.0 * (*num_long) / n);
    printf("  Ultra long chunks: %d - Block-cooperative with shared memory\n", *num_very_long_chunks);
    
    // Standard block calculation for short rows (one thread per row)
    ...
    *very_long_blocks = *num_very_long_chunks;
    ...
    printf("  Short rows: %d blocks x %d threads = %d total threads (direct processing)\n", 
           *short_blocks, threads, *short_blocks * threads);
    printf("  Long rows: %d blocks x %d threads (%d warps) with shared memory\n", 
           *long_blocks, threads, *long_blocks * (threads / WARP_SIZE));
    printf("  Ultra long chunks: %d blocks x %d threads with shared memory\n", *very_long_blocks, threads);
    printf("  Total blocks: %d\n", total_blocks);
    printf("  NOTE: Shared memory used only for long and very long rows\n");
    printf("\n");
}

__global__ void spmv_optimized_shared(const double *__restrict__ csr_values, 
    ...
    double *shared_values = (double*)shared_mem;
    int *shared_cols = (int*)&shared_values[SHARED_MEM_ELEMENTS];
    ...
    if (block_id < short_blocks) {
        // Short rows - simple direct processing without shared memory
        ...
        // Direct computation for short rows - no shared memory overhead
        #pragma unroll 4
        ...
        // Long rows - warp-cooperative processing with shared memory
        ...
        if (warp_global_id < num_long) {
            ...
            // Use shared memory with warp cooperation for long rows
            if (row_length <= SHARED_MEM_ELEMENTS) {
                // Warp cooperatively loads row data into shared memory
                int warp_offset = warp_id * WARP_SIZE;
                
                for (int i = warp_lane; i < row_length; i += WARP_SIZE) {
                    if (warp_offset + i < SHARED_MEM_ELEMENTS) {
                        shared_values[warp_offset + i] = csr_values[start + i];
                        shared_cols[warp_offset + i] = csr_col_indices[start + i];
                    }
                }
                
                __syncwarp();
                
                // Process from shared memory with warp cooperation
                for (int i = warp_lane; i < row_length; i += WARP_SIZE) {
                    if (warp_offset + i < SHARED_MEM_ELEMENTS) {
                        thread_sum += shared_values[warp_offset + i] * __ldg(&vec[shared_cols[warp_offset + i]]);
                    }
                }
                
                // Handle any remaining elements that didn't fit in shared memory
                int remaining_start = start + SHARED_MEM_ELEMENTS;
                for (int idx = remaining_start + warp_lane; idx < end; idx += WARP_SIZE) {
                    thread_sum += csr_values[idx] * __ldg(&vec[csr_col_indices[idx]]);
                }
            } else {
                // For very long rows that don't fit in shared memory, process directly
        ...
        // Very long rows - block-level processing with shared memory and warp cooperation
        ...
        // Process in shared memory chunks with warp cooperation
        int elements_per_chunk = SHARED_MEM_ELEMENTS;
        int num_chunks = (row_length + elements_per_chunk - 1) / elements_per_chunk;
        
        for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
            int chunk_start = start + chunk_idx * elements_per_chunk;
            int chunk_end = min(chunk_start + elements_per_chunk, end);
            int chunk_length = chunk_end - chunk_start;
            
            // Warp cooperative loading into shared memory
            for (int i = thread_id; i < chunk_length; i += blockDim.x) {
                shared_values[i] = csr_values[chunk_start + i];
                shared_cols[i] = csr_col_indices[chunk_start + i];
            }
            
            __syncthreads();
            
            // Process data from shared memory with warp cooperation
            for (int i = warp_lane; i < chunk_length; i += WARP_SIZE) {
                thread_sum += shared_values[i] * __ldg(&vec[shared_cols[i]]);
            }
        ...
        // Store warp results in shared memory for block reduction
        if (warp_lane == 0) {
            shared_values[warp_id] = thread_sum;
        }
        ...
        if (warp_id == 0) {
            double warp_sum = (warp_lane < (blockDim.x >> 5)) ? shared_values[warp_lane] : 0.0;
            
            #pragma unroll
            for (int stride = 16; stride > 0; stride >>= 1) {
                warp_sum += __shfl_down_sync(0xFFFFFFFF, warp_sum, stride);
            }
            
            if (warp_lane == 0) {
                atomicAdd(&res[row], warp_sum);
            }
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // Optimized shared memory size calculation
    size_t shared_mem_size = SHARED_MEM_ELEMENTS * sizeof(double) + SHARED_MEM_ELEMENTS * sizeof(int);
    printf("Shared memory allocation: %zu bytes (%d elements each for values and cols)\n", 
           shared_mem_size, SHARED_MEM_ELEMENTS);
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        if (total_kernel_blocks > 0) {
            spmv_optimized_shared<<<total_kernel_blocks, threads, shared_mem_size>>>(
                csr_vals, row_ptr, csr_cols, v, C, rows,
                d_short_rows, d_long_rows, d_very_long_chunks,
                num_short, num_long, num_very_long_chunks,
                short_blocks, long_blocks, very_long_blocks);
            checkCudaError("spmv_optimized_shared");
        }
