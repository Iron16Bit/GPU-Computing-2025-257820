...
// Based on Algorithm 8 from https://bora.uib.no/bora-xmlui/bitstream/handle/1956/8024/116259352.pdf?sequence=1&isAllowed=y
__global__
void spmv(int *Arows, int *Acols, double *Avals, double *v, double *C, int rows, int cols, int values, int *colOffsets, double *tmp_C) {
    int col_id = blockIdx.x * blockDim.x + threadIdx.x;
    
    if (col_id < cols) {
        double v_col = v[col_id];
        int start = colOffsets[col_id];
        int end = (col_id < cols - 1) ? colOffsets[col_id + 1] : values;

        // First phase: each thread computes its column contribution
    ...
    if (col_id < rows) { // Now we're computing output elements
        double sum = 0.0;
        for (int j = 0; j < cols; j++) {
            sum += tmp_C[col_id * cols + j];
        }
        C[col_id] = sum;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    double *tmp_C;
    cudaMallocManaged(&tmp_C, N*cols*sizeof(double));

    cudaEvent_t start, stop;

    for (int i=0; i<ITERATIONS; i++) {
        cudaMemset(C, 0, rows * sizeof(double));
        cudaMemset(tmp_C, 0, N*cols*sizeof(double));
        ...
        spmv<<<blocksPerGrid, threadsPerBlock>>>(Arows, Acols, Avals, v, C, rows, cols, values, colOffsets, tmp_C);
