...
int main(int argc, char *argv[]) {
    ...
    int blockSize = sqrt(threadsPerBlock);
    ...
    for (int i=0; i<ITERATIONS; i++) {
        ...
        spmv<<<blocksPerGrid, dim3(blockSize, blockSize), sharedMemSize>>>(Arows, Acols, Avals, v, C, rows, cols, values);
