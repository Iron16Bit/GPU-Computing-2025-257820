...
__global__
void matrix_multiplication_kernel_1D(double *A, double *B, double *C, int rows, int cols) {
...
int main(int argc, char *argv[]) {
    ...
    dim3 threadsPerBlock2D(16, 16);  // 16x16 = 256 threads per block
    dim3 numBlocks2D(
        (cols + threadsPerBlock2D.x - 1) / threadsPerBlock2D.x,
        (rows + threadsPerBlock2D.y - 1) / threadsPerBlock2D.y
    );
    ...
    matrix_multiplication_kernel<<<numBlocks2D, threadsPerBlock2D>>>(M, v, C, rows, cols);
    ...
    printf("[GPU naive 2D] Elapsed time: %f ms\n", milliseconds);
    ...
    // Perform GPU matrix-vector multiplication using 1D kernel
    int threadsPerBlock1D = 256;
    int numThreads = rows * cols;
    int numBlocks1D = (numThreads + threadsPerBlock1D - 1) / threadsPerBlock1D;
    ...
    // Run kernel with timing
    cudaEventRecord(start);
    matrix_multiplication_kernel_1D<<<numBlocks1D, threadsPerBlock1D>>>(M, v, C, rows, cols);
    cudaEventRecord(stop);
    ...
    printf("[GPU naive 1D] Elapsed time: %f ms\n", milliseconds);
    printf("Bandwidth: %f GB/s\n", calculateBandwidthGBs(rows, cols, milliseconds));
